...
{
    public partial class ModelRepoChannels(ModelPlugins modelPlugins) : ViewModelCollection<AircraftChannels, AircraftChannels>((modelPlugins?.PluginRepo?.Channels as IDictionary<string, AircraftChannels>).Values ?? [], (s) => s, (s) => s != null)
    {
        protected virtual ModelPlugins ModelPlugins { get; } = modelPlugins;
        public override ICollection<AircraftChannels> Source => (ModelPlugins?.PluginRepo?.Channels as IDictionary<string, AircraftChannels>).Values ?? [];
