...
namespace Any2GSX.Tools
{
    public static class WindowTools
    {
        public static string GetMsfsWindowTitle()
        {
            ...
            try
            {
                Process proc = Process.GetProcessesByName(Any2GSX.Instance.Config.BinaryMsfs2024)?.FirstOrDefault();
                if (!string.IsNullOrWhiteSpace(proc?.MainWindowTitle))
                    result = proc.MainWindowTitle;
                else
                    result = $"{Any2GSX.Instance.Config.Msfs2024WindowTitle}{proc?.MainModule?.FileVersionInfo?.FileVersion?.Replace(',', '.')}";
            }
            catch (Exception ex)
            {
