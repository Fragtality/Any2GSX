...
try {
	...
	######### PACKAGES
	if ($cfgBuildPackages -and $buildConfiguration -eq "Release") {
		Write-Host "Building Simulator Packages ..."
		$workPath = Join-Path $pathBase "MSFS2020-Package"
		cd $workPath
		.\BuildPackage.ps1

		$workPath = Join-Path $pathBase "MSFS2024-Package"
		cd $workPath
		.\BuildPackage.ps1
	}
